...
router.post('/send-otp-email', async (req, res) => {
    ...
    //Update user
    const updateOTP = await User.findByIdAndUpdate(user.id, { otp: otp }, { new: true });
    if (!updateOTP) {
...
router.post('/verify-email', async (req, res) => {
    ...
    //Update user
    const userUpdated = await User.findByIdAndUpdate(user.id, { email_verified: true }, { new: true });
    if (!userUpdated) {
        return res.status(500).json({ message: "Error verifying Email", status: "error" });
    }

    //Set otp to empty
    const userUpdatedOtp = await User.findByIdAndUpdate(user.id, { otp: "" }, { new: true });
    if (!userUpdatedOtp) {
        return res.status(500).json({ message: "Error verifying Email", status: "error" });
    }
...
router.post('/verify-phone', async (req, res) => {
    ...
    //Update user
    const userUpdated = await User.findByIdAndUpdate(user.id, { phone_verified: true }, { new: true });
    if (!userUpdated) {
...
router.post('/change-password', async (req, res) => {
    ...
    //Update user
    const userUpdated = await User.findByIdAndUpdate(user.id, { password: hashedPassword }, { new: true });
    if (!userUpdated) {
...
router.post('/forgot-password', async (req, res) => {
    ...
    //Update user
    const updateOTP = await User.findByIdAndUpdate(userFound._id, { otp: otp }, { new: true });
    if (!updateOTP) {
        return res.status(500).json({ message: "Error sending OTP", status: "error" });
    }
